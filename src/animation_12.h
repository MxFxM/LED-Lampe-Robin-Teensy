...
void adjust_gain_12(float *bin_all, float *gain, AudioAmplifier *amp, float stripe_maximums[NUMBER_OF_STRIPES], float *peak)
{
...
void update_peaks_12(float *bin_all, AudioAnalyzePeak *peak_all, float bins[NUMBER_OF_STRIPES], float *gain, AudioAmplifier *amp, AudioAnalyzeFFT1024 *fft1024, float stripe_maximums[NUMBER_OF_STRIPES])
{
    ...
    if (fft1024->available())
    {
        ...
        adjust_gain_12(bin_all, gain, amp, stripe_maximums, &bass);
...
void run_animation_12(RgbColor ledarray[NUMPIXELS], float bins[NUMBER_OF_STRIPES], int stripe_offsets[NUMBER_OF_STRIPES + 1], float stripe_maximums[NUMBER_OF_STRIPES])
{
    // prepare the color
    HsvColor hsvcol;
    hue_12 += HUE_CHANGE_SPEED_SLOW; // increase hue value for rainbow effect
    ...
    hsvcol.h = int(hue_12);
    hsvcol.s = 255;
    hsvcol.v = int(DEFAULT_BRIGHTNESS * bins[int(NUMBER_OF_STRIPES / 2)]); // modulate brightness with current base
