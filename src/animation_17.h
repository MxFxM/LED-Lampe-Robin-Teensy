...
void adjust_gain_17(float *bin_all, float *gain, AudioAmplifier *amp, float stripe_maximums[NUMBER_OF_STRIPES], float *peak)
{
...
void update_peaks_17(float *bin_all, AudioAnalyzePeak *peak_all, float bins[NUMBER_OF_STRIPES], float *gain, AudioAmplifier *amp, AudioAnalyzeFFT1024 *fft1024, float stripe_maximums[NUMBER_OF_STRIPES])
{
    ...
    if (fft1024->available())
    {
        ...
        adjust_gain_17(bin_all, gain, amp, stripe_maximums, &bass);
...
void run_animation_17(RgbColor ledarray[NUMPIXELS], float bins[NUMBER_OF_STRIPES], int stripe_offsets[NUMBER_OF_STRIPES + 1], float stripe_maximums[NUMBER_OF_STRIPES])
{
    // prepare the color
    HsvColor hsvcol;
    hue_17 += HUE_CHANGE_SPEED_SLOW; // increase hue value for rainbow effect
    ...
    for (int lednr = 0; lednr < LEDS_PER_STRIPE - multiple_rows_local; lednr += multiple_rows_local)
    {
        ...
        int turnonnr = map(bins_17[int(lednr / multiple_rows_local)], 0.0, stripe_maximums[0] * 2, 1, NUMBER_OF_STRIPES); // map to number of stripes
        ...
        for (int i = 0; i < turnonnr; i++)
        {
            ...
            {
                hsvcol.h = int(hue_17 + (lednr + multiple_rows) * 2);
                hsvcol.s = 255;
                hsvcol.v = DEFAULT_BRIGHTNESS;
