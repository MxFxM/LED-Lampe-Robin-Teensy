...
void adjust_gain_5(float *bin_all, float *gain, AudioAmplifier *amp)
{
...
void update_peaks_5(float *bin_all, AudioAnalyzePeak *peak_all, float bins[NUMBER_OF_STRIPES], float *gain, AudioAmplifier *amp)
{
    ...
    {
        peak = peak_all->read();
        adjust_gain_5(bin_all, gain, amp);
...
void run_animation_5(RgbColor ledarray[NUMPIXELS], float bins[NUMBER_OF_STRIPES], int stripe_offsets[NUMBER_OF_STRIPES + 1])
{
    // prepare the color
    HsvColor hsvcol;
    hue_5 += HUE_CHANGE_SPEED_SLOW; // increase hue value for rainbow effect
    ...
    for (int stripenr = 0; stripenr < NUMBER_OF_STRIPES; stripenr++)
    {
        ...
        int turnonnr = map(bins[stripenr], 0.0, 2.0, 1, LEDS_PER_STRIPE); // map to number of pixels // modulo to account for 3 strips only at the moment
