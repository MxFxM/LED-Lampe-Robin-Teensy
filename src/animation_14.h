...
void adjust_gain_14(float *bin_all, float *gain, AudioAmplifier *amp, float stripe_maximums[NUMBER_OF_STRIPES], float *peak)
{
...
void update_peaks_14(float *bin_all, AudioAnalyzePeak *peak_all, float bins[NUMBER_OF_STRIPES], float *gain, AudioAmplifier *amp, AudioAnalyzeFFT1024 *fft1024, float stripe_maximums[NUMBER_OF_STRIPES])
{
    ...
    if (fft1024->available())
    {
        ...
        adjust_gain_14(bin_all, gain, amp, stripe_maximums, &bass);
...
void run_animation_14(RgbColor ledarray[NUMPIXELS], float bins[NUMBER_OF_STRIPES], int stripe_offsets[NUMBER_OF_STRIPES + 1], float stripe_maximums[NUMBER_OF_STRIPES])
{
    // prepare the color
    HsvColor hsvcol;
    hue_14 += HUE_CHANGE_SPEED_SLOW; // increase hue value for rainbow effect
    ...
    for (int stripenr = 0; stripenr < NUMBER_OF_STRIPES; stripenr++)
    {
        ...
        // adjust hsv color
        if (stripenr < int(floor(NUMBER_OF_STRIPES / 2)) - 1)
        {
            hsvcol.h = int(hue_14 + 20 * (7 - stripenr));
            hsvcol.s = 255;
            hsvcol.v = DEFAULT_BRIGHTNESS;
        }
        if (stripenr > int(ceil(NUMBER_OF_STRIPES / 2)))
        {
            hsvcol.h = int(hue_14 + 20 * (stripenr - 8));
            hsvcol.s = 255;
            hsvcol.v = DEFAULT_BRIGHTNESS;
        }
