...
void adjust_gain_9(float *bin_all, float *gain, AudioAmplifier *amp, float stripe_maximums[NUMBER_OF_STRIPES], float *peak)
{
...
void update_peaks_9(float *bin_all, AudioAnalyzePeak *peak_all, float bins[NUMBER_OF_STRIPES], float *gain, AudioAmplifier *amp, AudioAnalyzeFFT1024 *fft1024, float stripe_maximums[NUMBER_OF_STRIPES])
{
    ...
    if (fft1024->available())
    {
        ...
        adjust_gain_9(bin_all, gain, amp, stripe_maximums, &bass);
...
void run_animation_9(RgbColor ledarray[NUMPIXELS], float bins[NUMBER_OF_STRIPES], int stripe_offsets[NUMBER_OF_STRIPES + 1], float stripe_maximums[NUMBER_OF_STRIPES])
{
    // prepare the color
    HsvColor hsvcol;
    hue_9 += HUE_CHANGE_SPEED_SLOW; // increase hue value for rainbow effect
